...
class OptimizationRepository:
    ...
    def calculate_current_cost(self, product_quantities: Dict[int, int]) -> Dict[str, Any]:
        ...
        for provider in all_providers:
            ...
            for product_id, credit_files in product_quantities.items():
                ...
                # Build allocation details for base
                ...
                for item_id in item_ids:
                    ...
                    if item_id not in allocation_details[product_id]['items']:
                        ...
                        for pa in item_alloc.get('providers', []):
                            ...
                            prov = self.pricing_repo.get_provider(prov_id)
                            providers_alloc.append({
                                'provider_id': prov_id,
                                'provider_name': prov.company_name if prov else '',
                    ...
                    # Determine assigned files using actual allocations
                    item_alloc = allocations.get(item_id) or allocations.get(str(item_id)) or {}
    ...
    def get_current_allocations(self, product_quantities: Dict[int, int]) -> Dict[int, int]:
        ...
        for product_id in product_quantities.keys():
            item_ids = self.products_repo.get_item_ids_for_product(product_id)
            product_allocations = self.products_repo.get_allocations_for_product(product_id)
            
            for item_id in item_ids:
                if item_id in allocations:
                    continue
                
                # Get allocation from product_item_allocations table
    ...
    def calculate_cost_with_allocations(
        ...
        for provider in all_providers:
            ...
            for product_id, credit_files in product_quantities.items():
                ...
                # Build allocation details for optimized (single-provider per item)
                if product_id not in allocation_details:
                    allocation_details[product_id] = {
                        'product_name': product.name,
                        'items': {}
                ...
                # Support nested optimized allocations: {product_id: {items: {item_id: {mode, allocations:[{provider_id, value}]}}}}
                ...
                for item_id in item_ids:
                    ...
                    # Optimized allocation details (mirror provided allocations)
                    ...
                    if isinstance(item_alloc_def, dict) and item_id not in allocation_details[product_id]['items']:
                        ...
                        for pa in item_alloc_def.get('allocations', []):
                            prov = self.pricing_repo.get_provider(pa.get('provider_id'))
                            allocs.append({
                    ...
                    # Determine assigned files for this provider
                    assigned_files = 0.0
                    if isinstance(item_alloc_def, dict) and 'allocations' in item_alloc_def:
                        ...
                        # Backward compatibility: flat mapping item_id -> provider_id
